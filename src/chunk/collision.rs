...
pub fn calculate_collider(
                ...
                if let Some(block_id) = chunk.get(&(x, y, z)) {
                    if let Some(block) = config.dict.get(block_id) {
                        if !block.transparent {
                            collider_data.push((Isometry::translation(x as f32, y as f32, z as f32), SharedShape::cuboid(0.5, 0.5, 0.5)));
                        }
                    }
                } 
            }
    ...
    ColliderBuilder::compound(collider_data).translation(translation).build()
}
