...
use crate::world::{GameData, Event};

use super::generation::load_chunk;
use super::meshing::mesh_chunk;
use super::{chunk_position, player_to_position, chunk_id, calc_lod};
...
pub fn load_world(
    ...
    let (i, j, k) = chunk_position(&data.chunk_config, &player_to_position(&data.player.position));
        ...
        for y in (j - radius)..(j + radius) {
            for z in (k - radius)..(k + radius) {
                ...
                if !data.loaded_chunks.contains_key(&chunk_id)
                    && !data.loading.contains(&chunk_id)
                {
    ...
    for (chunk_id, chunk_pos) in chunks_to_load.into_iter() {
        ...
        thread_pool.spawn(move || {
                ...
                let mesh = mesh_chunk(&chunk, &config, &chunk_pos, calc_lod());
                
                let mut done_loading = done_loading.lock(5).unwrap();
                done_loading.insert(chunk_id, (chunk_pos, chunk, mesh));
        })
    }
...
pub fn check_done_load_world(
    ...
    for (chunk_id, (chunk_pos, chunk, mesh)) in done_loading.drain() {
